...
from utils.util import count_chinese_chars
from utils.util_request import request_get
...
def get_tencent_hot_ranking_list(news_num: int, image_dir_path: Path) -> List[News]:
    ...
    # print("news_list_without_content:\n", news_list_without_content)
...
if __name__ == '__main__':
    from util_news import write_news_json
    ...
    image_dir_path = r"G:\zwj\WL\LLM\NewsHot\IMAGES"
    image_dir_path = Path(image_dir_path)
    news_list = get_tencent_hot_ranking_list(1, image_dir_path)
    print(news_list)
    news_json_path = Path(r"G:\zwj\WL\LLM\NewsHot\IMAGES\news.json")
    write_news_json(news_list, news_json_path)
